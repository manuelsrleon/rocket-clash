...
public class PrometeoCarController : MonoBehaviour
{
    ...
    {
      //CONTROLS
      float throttleValue = throttleAction.ReadValue<float>();
      float steerValue = steerAction.ReadValue<float>();
      float brakeValue = brakeAction.ReadValue<float>();
      float handbrakeValue = handbrakeAction.ReadValue<float>();
      ...
      */
        ...
        if(throttleValue == 0 && handbrakeValue == 0 && !deceleratingCar){
          InvokeRepeating("DecelerateCar", 0f, 0.1f);
          deceleratingCar = true;
        }

        if(steerValue == 0 && steeringAxis != 0f){
          ResetSteeringAngle();
        }
